...
import { fetchMyPayrollDocuments, uploadGlobalPayrollDocument, deletePayrollDocument, type PayrollDocument } from '@/services/payroll.service'
import { sendGlobalNotification } from '@/services/notifications.service'
...
async function load() {
  docs.value = (await fetchMyPayrollDocuments()).filter(d => d.scope === 'global')
}
onMounted(load)
...
async function upload() {
  ...
  try {
    await uploadGlobalPayrollDocument(selectedFile.value)
    await sendGlobalNotification('📄 Nuevo documento de nómina', `Se subió un nuevo documento: ${selectedFile.value.name}`, 'payroll')
...
<template>
  <div>
    <h1 class="title">Nómina general</h1>
    <BaseCard class="section">
      <p class="hint">Visible para TODOS los usuarios de la plataforma. Se borra automáticamente a los 5 días.</p>
      <BaseAlert v-if="error" tone="danger">{{ error }}</BaseAlert>
      <BaseAlert v-if="saved" tone="success">Documento subido y notificación enviada.</BaseAlert>
